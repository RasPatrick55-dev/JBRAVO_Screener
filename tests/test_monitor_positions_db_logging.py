...
class TestMonitorDbLogging(unittest.TestCase):
    ...
    def test_sell_fill_logs_and_closes_trade(self):
        ...
        with mock.patch.object(monitor, "db_logging_enabled", return_value=True), \
            mock.patch.object(monitor.db, "insert_order_event", return_value=True) as insert_event, \
            mock.patch.object(monitor.db, "insert_executed_trade", return_value=True) as insert_exec, \
            mock.patch.object(monitor.db, "close_trade_on_sell_fill", return_value=True) as close_trade, \
